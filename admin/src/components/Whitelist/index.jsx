            ...
            <CustomTable colCount={5} rowCount={users.length}>
              <Thead>
                <Tr>
                  <Th>
                    {
                      formatMessage(getTrad('whitelist.table.no'))
                    }
                  </Th>
                  <Th>
                    {
                      formatMessage(getTrad('whitelist.table.email'))
                    }
                  </Th>
                  <Th>
                    {
                      formatMessage(getTrad('whitelist.table.roles'))
                    }
                  </Th>
                  <Th>
                    {
                      formatMessage(getTrad('whitelist.table.created'))
                    }
                  </Th>
                    ...
                    return (
                      <Tr key={user.email}>
                        ...
                        <Td style={{ paddingRight: 0 }}>
                          <Flex justifyContent="flex-end" onClick={(e) => e.stopPropagation()} style={{ width: '100%' }}>
                            <Dialog.Root>
                              <Dialog.Trigger>
                                <IconButton label={formatMessage(getTrad('whitelist.delete.label'))} withTooltip={false}><Trash/></IconButton>
                              </Dialog.Trigger>
                              <Dialog.Content>
                                <Dialog.Header>
                                  {
                                ...
                                <Dialog.Footer>
                                  ...
                                  <Dialog.Action>
                                    <Button fullWidth variant="danger-light" onClick={() => onDelete(user.email)}>
                                      {
              ...
              <Box paddingTop={4}>
                <Flex justifyContent="flex-end">
                  <Pagination activePage={page} pageCount={pageCount}>
                    <PreviousLink href="#" onClick={(e) => { e.preventDefault(); setPage(p => Math.max(1, p - 1)); }}>Go to previous page</PreviousLink>
                    {Array.from({ length: pageCount }).map((_, i) => (
                      <PageLink key={i + 1} number={i + 1} href="#" onClick={(e) => { e.preventDefault(); setPage(i + 1); }}>
                        Go to page {i + 1}
                      </PageLink>
                    ))}
                    <NextLink href="#" onClick={(e) => { e.preventDefault(); setPage(p => Math.min(pageCount, p + 1)); }}>Go to next page</NextLink>
                  </Pagination>
                </Flex>
              </Box>
            )}
