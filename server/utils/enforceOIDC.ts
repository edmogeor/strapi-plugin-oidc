...
// Parse the OIDC_ENFORCE config value into a boolean or null. Returns `true` or `false` when the config explicitly sets enforcement, or `null` when it should fall through to the database setting.
...
// Resolve the effective enforceOIDC value by preferring the config override and falling back to the database setting.
