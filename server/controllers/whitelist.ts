...
async function register(ctx: Context) {
  ...
  const results = await Promise.all(
    [...new Set(validEmails)].map(async (singleEmail) => {
      if (await whitelistService.hasUser(singleEmail)) return 'alreadyWhitelisted';
      await whitelistService.registerUser(singleEmail);
      return 'accepted';
    }),
  );
  const acceptedCount = results.filter((result) => result === 'accepted').length;
  const alreadyWhitelistedCount = results.length - acceptedCount;
