...
- **SSO button now works independently of skipLoginPage** — The sign-in controller only gates auto-redirects (`?oidc_redirect=1`), not explicit SSO button clicks. This means the "Login via SSO" button works on the login page even when "Skip Login Page" is toggled off.
- **CI warnings** — Cleared legacy `always-auth` npm config in both `ci.yml` and `release.yml` workflows.
...
- **Dependency updates** — Bumped semver-compatible deps; ran `npm audit fix`. Vulnerabilities reduced from 38 to 23 (remaining are in `@strapi/strapi` transitive dependencies).
...
- **Trailing slashes stripped from `OIDC_PUBLIC_URL`** — Prevents double slashes in the callback URL when users include a trailing slash.
...
- **Renamed `OIDC_REDIRECT_URI` to `OIDC_PUBLIC_URL`** — Users now provide just their Strapi origin (e.g. `https://myapp.com`). The plugin appends `/strapi-plugin-oidc/oidc/callback` automatically. Falls back to the `PUBLIC_URL` environment variable if unset. In production, one of the two must be set; in development, defaults to `http://localhost:1337`.
- **README updates** — `OIDC_PUBLIC_URL` moved to first required config item; verbose Skip Login Page section removed; API examples use `https://strapi.example.com`.
...
- **Login page flash when `OIDC_SKIP_LOGIN_PAGE` is enabled** — The client now redirects immediately from auth routes (before React renders) using `window.location.replace()`. The server-side sign-in controller gates whether to proceed with OIDC or bounce back to the login page with `?oidc_redirect=1`, which the client detects to break the redirect loop. This eliminates the flash entirely by never mounting the login form when a redirect is needed.
