...
- **Renamed `OIDC_REDIRECT_URI` to `STRAPI_URL`** — Users now provide just their Strapi origin (e.g. `https://myapp.com`). The plugin appends `/strapi-plugin-oidc/oidc/callback` automatically. The old full-URL format is no longer supported.
- **README updates** — `STRAPI_URL` moved to first required config item; verbose Skip Login Page section removed.
